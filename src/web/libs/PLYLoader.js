...
class PLYLoader extends Loader {
	...
	parse(data) {
		...
		function createBuffer() {
			...
			for (const customProperty of Object.keys(
				scope.customPropertyMapping
			)) {
		...
		function parseASCII(data, header) {
			...
			loop: for (let i = 0; i < header.elements.length; i++) {
				...
				const attributeMap = mapElementAttributes(
					elementDesc.properties
				);
		...
		function handleElement(buffer, elementName, element, cacheEntry) {
			if (elementName === "vertex") {
				...
				if (cacheEntry.attrS !== null && cacheEntry.attrT !== null) {
					buffer.uvs.push(
						element[cacheEntry.attrS],
						element[cacheEntry.attrT]
					);
				}
